...
class PolicyService():
    ...
    def show_policy_menu(self):
        ...
        while not svm_chosen or not name_chosen or not schedule_chosen or not count_chosen:
            ...
            menu = TerminalMenu(options, title="Create a policy", menu_cursor_style=("fg_blue", "bold"))
            selection_i = menu.show()
            
            if selection_i == 0:
                options = [x[0] for x in self.svms]
                menu = TerminalMenu(options, title="Select an SVM", search_key='\\', menu_cursor_style=("fg_blue", "bold"))
                selection_i = menu.show()
                ...
                menu = TerminalMenu(options, title="Select schedule", search_key='\\', menu_cursor_style=("fg_blue", "bold"))
                selection_i = menu.show()
